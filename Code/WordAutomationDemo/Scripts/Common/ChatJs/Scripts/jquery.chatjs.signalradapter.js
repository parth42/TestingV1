...
SignalRAdapter.prototype = {
    init: function (chat, done) {
        ...
        if (_this.hub) {
            _this.hub.client.sendMessage = function(message, firstlastName) {
                chat.client.sendMessage(message, firstlastName);
            };
            ...
            _this.hub.client.clearMsgHistory = function(myid, othersid) {
                chat.client.clearMsgHistory(myid, othersid);
            };

            _this.hub.client.usersListChanged = function(usersList, OfflineMsgUsers) {
                //alert(TempVarbl);
                chat.client.usersListChanged(usersList, OfflineMsgUsers);
            };

            _this.hub.client.msgOfflineList = function(OfflineMsgUsers) {
                //alert(TempVarbl);
                chat.client.msgOfflineList(OfflineMsgUsers);
            };
            ...
            _this.server.clearMsgHistory = function(myid, othersid, done) {
                /// <summary>Sends a typing signal to the server</summary>
                /// <param FullName="otherUserId" type="Number">The id of the user to which the typing signal is being sent</param>
                /// <param FullName="done" type="Function">Function to be called when this method completes</param>
                _this.hub.server.clearMsgHistory(myid, othersid).done(function(result) {
                    if (done)
                        done(result);
                });
            };
            ...
            _this.server.getOffUsersMsgList = function(done) {
                /// <summary>Gets the list of the users in the current room</summary>
                /// <param FullName="otherUserId" type="Number">The id of the user from which you want the information</param>
                /// <param FullName="done" type="Function">FUnction to be called when this method completes</param>
                _this.hub.server.getOffUsersMsgList().done(function(result) {
                    if (done)
                        done(result);
                });
            };
